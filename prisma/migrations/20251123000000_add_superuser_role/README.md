...
1. Updates existing `User` records with `role = 'ADMIN'` to `role = 'SYSTEM_ADMIN'`
2. This preserves the intent of system-level administrative access
3. Schema comments added to document the role hierarchy

## Authorization Rules

- **Acceptance & Engagement Letter Approval**: SYSTEM_ADMIN OR Partner (ServiceLineUser.role = ADMIN for project's service line)
- **Service Line Access Check**: System Admins bypass; Partners/Managers/Staff require service line access
- **Project Access**: All users must be assigned to projects via ProjectUser

## Impact

- Existing system administrators will automatically become SYSTEM_ADMINs
